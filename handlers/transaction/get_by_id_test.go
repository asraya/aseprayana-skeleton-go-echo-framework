...
func Test_domainHandler_GetById(t *testing.T) {
	type fields struct {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err := b.GetById(tt.args.c); (err != nil) != tt.wantErr {
				t.Errorf("domainHandler.GetById() error = %v, wantErr %v", err, tt.wantErr)
			}
		})
	}
}
